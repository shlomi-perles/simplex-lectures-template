...
For OpenGL-rendered 3D scenes, use the same `ThreeDScene` camera methods. Add `--renderer opengl --write_to_movie` on the CLI or set `config.renderer = "opengl"` before constructing mobjects.
