...
> **v0.20 note:** `MathTex` was substantially rewritten for more robust splitting. Indices into `eq[...]` are now consistent with the LaTeX source, and named groups in any SVG file are accessible via `SVGMobject` (`svg["group_id"]`). Older code that relied on counting whitespace submobjects may need to be re-indexed.
...
**Why double braces?** Single braces `{ }` already have meaning in LaTeX (grouping). Manim uses `{{ }}` as its own delimiter for splitting into submobjects. Keep the groups clean and avoid nesting `{{ }}` inside LaTeX commands. If you need literal braces, escape them as `\{` and `\}`.
...
## Setting class-wide defaults (v0.19+)

`Animation.set_default(...)` lets you change defaults for an animation class once instead of repeating kwargs. Useful when every `Write` in a scene should share the same `run_time` or every `FadeIn` should `shift=UP * 0.3`:

```python
class StyledScene(Scene):
    def construct(self):
        Write.set_default(run_time=1.5)
        FadeIn.set_default(shift=UP * 0.3, lag_ratio=0.05)

        eq = MathTex(r"E = mc^2")
        self.play(Write(eq))            # uses run_time=1.5
        self.play(FadeIn(Text("done"))) # uses shift=UP*0.3
```

Resets are not automatic — call `Write.set_default()` (no args) to clear, or set explicit kwargs again.
